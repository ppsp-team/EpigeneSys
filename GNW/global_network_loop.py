...
def sim(cnt, rate_num, ratio, trace):
    ...
    with open('data{}_rate{}_ratio{}_cond{}.pickle'.format(cnt, rate_num, ratio, trace), 'wb') as f:
        pickle.dump(dump, f)
        ...
        for rate_num in range(0, 11):
            for ratio in range(10, 101, 10):
                for trace in [True, False]:  # False = Delay conditioning
                    params.append((cnt, rate_num, ratio, trace))


    Parallel(n_jobs=-2, backend="multiprocessing")(delayed(sim)(cnt, rate_num, ratio, trace) for cnt, rate_num, ratio, trace in params)