    ...
    for rate_num in range(11):
        for ratio in range(5, 101, 5):
            for trace in [True, False]:  # False = Delay conditioning
                filename = 'output/data{}_rate{}_ratio{}_cond{}.pickle'.format(sim_num, rate_num, ratio, trace)
...
df.to_csv('output/aggregated_results_additional.csv', index=False)


# score, var_fin, learn_time, n_selected, n_pruned = extract_data('output/data0_rate1_ratio5_condTrue.pickle')
